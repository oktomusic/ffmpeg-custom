...
RUN PKG_CONFIG_PATH=/$(xx-info sysroot)usr/local/lib/pkgconfig CC=xx-clang CXX=xx-clang++ PKG_CONFIG_ALL_STATIC=1 LDFLAGS="-static" ./configure --prefix=/usr/local --arch=$(xx-info arch) --target-os=linux \
    --disable-everything \
    # Build configuration
    --disable-shared \
    --enable-static \
